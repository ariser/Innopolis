...
public class ProblemA {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Graph<String, Integer> graph = Graph.fromString(str, String.class);

            for (Graph.Vertex<String, Integer> v : graph.getSortedAdjacent(graph.getVertex("Donetsk-DU"))) {
                result += v.getElement() + " ";
            }
            result += "\n";
            for (Graph.Vertex<String, Integer> v : graph.getSortedAdjacent(graph.getVertex("Kiev-U"))) {
                result += v.getElement() + " ";
            }
            result += "\n";
            for (Graph.Vertex<String, Integer> v : graph.getSortedAdjacent(graph.getVertex("Lviv-U"))) {
                result += v.getElement() + " ";
            }
            result += "\n";
            for (Graph.Vertex<String, Integer> v : graph.getSortedAdjacent(graph.getVertex("Batumi-G"))) {
                result += v.getElement() + " ";
            }
            result += "\n";
            for (Graph.Vertex<String, Integer> v : graph.getSortedAdjacent(graph.getVertex("Rostov-R"))) {
                result += v.getElement() + " ";
            }
    ...
    public static class Graph<E extends Comparable, W extends Comparable<W>> {
        private List<Vertex<E, W>> vertices = new ArrayList<>();
        private List<Edge<E, W>> edges = new ArrayList<>();

        public static <E extends Comparable> Graph<E, Integer> fromString(String source, Class<E> elementClass)
        ...
        public List<Vertex<E, W>> getSortedAdjacent(Vertex<E, W> vertex) {
            List<Vertex<E, W>> adjacent = getAdjacent(vertex);
            Sort.mergeSort(adjacent);
            return adjacent;
        }
        ...
        protected static class Edge<E extends Comparable, W> {
        ...
        public static class Vertex<E extends Comparable, W> implements Comparable<Vertex<E, W>> {
            protected E element;
            protected int position = -1;
            ...
            @Override
            public int compareTo(Vertex<E, W> o) {
