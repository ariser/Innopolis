...
public class ProblemD {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            graph.BFS(vertex -> {
                for (Graph.Vertex<String, Integer> adjacent : graph.getAdjacent(vertex)) {
                    if ((
                            (vertex.getElement().endsWith("-R") && adjacent.getElement().endsWith("-DU"))
                                    || (adjacent.getElement().endsWith("-R") && vertex.getElement().endsWith("-DU"))
                    )
                            || (
                            (vertex.getElement().endsWith("-R") && adjacent.getElement().endsWith("-DG"))
                                    || (adjacent.getElement().endsWith("-R") && vertex.getElement().endsWith("-DG"))
                    )
                            ) {
                        graph.removeEdge(vertex, adjacent, false);
                    }
                }
            });

            final int[] distance = {0};
            String home = "Rostov-R";
            final boolean[] done = {false};
            graph.BFS(graph.getVertex("Melitopol-U"), vertex -> {
                if (!done[0]) {
                    if (vertex.getElement().equals(home)) {
                        done[0] = true;
                        Graph.Vertex<String, Integer> parent = vertex;
                        while (parent != null) {
                            path[parent.distance] = parent.getElement();
                            parent = parent.parent;
                        }
                        distance[0] = vertex.distance;
                    }
                }
            });
            ...
            for (int i = 0; i < 10; i++) path[i] = "";
            graph.BFS(graph.getVertex("Sukhumi-DG"), vertex -> {
                if (!done[0]) {
                    if (vertex.getElement().equals("Lugansk-DU")) {
                        done[0] = true;
                        Graph.Vertex<String, Integer> parent = vertex;
                        while (parent != null) {
                            path[parent.distance] = parent.getElement();
    ...
    private static void writeResult(String result) {
        File out = new File("travel-now.txt");
        if (!out.exists()) {
            try {
                out.createNewFile();
            } catch (IOException e) {
    ...
    public static class Graph<E, W extends Comparable<W>> {
